...
func (context *chainContext) GetHeader(_ context.Context, hash common.Hash, number uint64) *types.Header {
		...
		go func() {
			...
			for task := range taskCh {
				var (
					...
					blockCtx = core.NewEVMBlockContext(nil, task.block.Header(), api.chainContext(ctx), nil)
				)
...
func (api *API) IntermediateRoots(ctx context.Context, hash common.Hash, config *TraceConfig) ([]common.Hash, error) {
	...
	var (
		...
		vmctx              = core.NewEVMBlockContext(nil, block.Header(), api.chainContext(ctx), nil)
...
func (api *API) traceBlock(ctx context.Context, block *types.Block, config *TraceConfig) ([]*txTraceResult, error) {
	...
	var (
		...
		blockCtx  = core.NewEVMBlockContext(nil, block.Header(), api.chainContext(ctx), nil)
		signer    = types.MakeSigner(api.backend.ChainConfig(), block.Number())
